...
def create_temp_link(real_url, expire_seconds=600):
    ...
    links[token] = (real_url, expire_time)
    return f"https://{os.environ.get('KOYEB_DOMAIN','yourdomain.com')}/go?f={token}"
...
# ----------------- تابع استخراج لینک واقعی با Headers -----------------
def get_download_link(search_query):
    query = search_query.replace(" ", "+")
    search_url = f"https://donyayeserial.io/?s={query}"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Referer": "https://donyayeserial.io/"
    }
    ...
    r2 = requests.get(link_page, headers=headers, timeout=10)
    if r2.status_code != 200:
        return None
    soup2 = BeautifulSoup(r2.text, "html.parser")
    download_btn = soup2.find("a", class_="download-btn")
    if not download_btn:
        return None
